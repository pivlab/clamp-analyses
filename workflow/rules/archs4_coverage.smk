...
A4_COV_DATABASES = list(A4_COV_CFG["databases"])
...
# Builds the path for one ARCHS4 coverage cell.
...
# Builds the path for one fitted coverage model.
...
# Looks up the reproducible random seed for a coverage run.
def a4_cov_rng_seed(seed):
    return A4_COV_RNG_SEEDS[int(seed) - 1]

# Looks up the first memory request for a coverage level.
def a4_cov_initial_mem(fraction):
    return int(A4_COV_CFG["resources"][str(fraction)]["mem_mb"])

# Increases memory for a retried coverage fit without exceeding the cap.
...
# Returns the configuration for one ORA database.
def a4_cov_db(wildcards):
    return A4_CFG["ora"]["databases"][wildcards.database]

# Returns the CLAMPbase loading matrix for a coverage dataset.
...
# Selects the sample provenance argument required by ORA.
...
# Returns every configured seed for a coverage level.
def a4_cov_seeds_for(fraction):
    return A4_COV_SEEDS

# Returns the filtered FBM or cell-specific FBM used for a coverage fit.
def a4_cov_fbm(fraction, seed):
    if int(fraction) == 100:
        return f"{A4_PREP}/fbm_filtered.bk"
    return f"{a4_cov_cell(fraction, seed)}/fbm_subsampled.bk"

A4_COV_CELLS = {
    a4_cov_cell(fraction, seed): (fraction, seed)
    for fraction in A4_COV_LEVELS
    for seed in a4_cov_seeds_for(fraction)
}
A4_COV_CELL_PATTERN = "|".join(re.escape(cell) for cell in A4_COV_CELLS)

# Returns the fraction associated with a coverage-cell path.
def a4_cov_fraction_of(cell):
    return A4_COV_CELLS[str(cell)][0]

# Returns the seed associated with a coverage-cell path.
def a4_cov_seed_of(cell):
    return A4_COV_CELLS[str(cell)][1]


A4_COV_BP_ROOT = A4_COV_CFG["bp_models_root"]
A4_COV_BP_DATASETS = list(A4_COV_CFG["bp_model_datasets"])
A4_COV_BP_MANIFESTS = [
    f"{A4_COV_BP_ROOT}/{dataset}/manifest.json" for dataset in A4_COV_BP_DATASETS
]
A4_COV_CELL_TARGETS = [f"{cell}/CLAMPbase.rds" for cell in A4_COV_CELLS]
...
rule subsample_bp_coverage_cell:
    ...
    output:
        info=f"{{cell}}/subsample_info.rds",
        metadata=f"{{cell}}/metadata_cell.rds",
        samples=f"{{cell}}/samples.rds",
    log:
        f"{{cell}}/subsample.log"
    params:
        out_dir=lambda wildcards, output: os.path.dirname(output.info),
        fraction=lambda wc: a4_cov_fraction_of(wc.cell),
        seed=lambda wc: a4_cov_rng_seed(a4_cov_seed_of(wc.cell)),
        run_index=lambda wc: a4_cov_seed_of(wc.cell),
        sc_max=A4_CFG["preprocess"]["single_cell_probability_max"],
        block_size=A4_CFG["raw_data"]["block_size"],
    resources:
        mem_mb=lambda wc: a4_cov_initial_mem(a4_cov_fraction_of(wc.cell)),
        runtime=5760,
    wildcard_constraints:
        cell=A4_COV_CELL_PATTERN,
    conda: "clamp-analyses"
    shell:
        "Rscript {input.script} --h5 {input.h5} --metadata-filtered {input.metadata} "
        "--fbm-filtered {input.fbm} --out-dir {params.out_dir} "
        "--fraction {params.fraction} --seed {params.seed} --run-index {params.run_index} "
        "--single-cell-probability-max {params.sc_max} --block-size {params.block_size} "
        "> {log} 2>&1"


rule svd_bp_coverage_cell:
    input:
        metadata=rules.subsample_bp_coverage_cell.output.metadata,
        fbm=lambda wc: a4_cov_fbm(a4_cov_fraction_of(wc.cell), a4_cov_seed_of(wc.cell)),
        script="scripts/archs4/svd.R",
        common="scripts/archs4/common.R",
    output:
        svd=f"{{cell}}/svd.rds",
        k=f"{{cell}}/CLAMP_K.rds",
    log:
        f"{{cell}}/svd.log"
    params:
    ...
    shell:
        ...
        "--svd {output.svd} --k {output.k} --n-cores {params.n_cores} "
        "--k-multiplier {params.k_multiplier} --seed {params.seed} "
        "> {log} 2>&1"


rule clampbase_bp_coverage_cell:
    input:
        metadata=rules.subsample_bp_coverage_cell.output.metadata,
        samples=rules.subsample_bp_coverage_cell.output.samples,
        fbm=lambda wc: a4_cov_fbm(a4_cov_fraction_of(wc.cell), a4_cov_seed_of(wc.cell)),
        svd=rules.svd_bp_coverage_cell.output.svd,
        k=rules.svd_bp_coverage_cell.output.k,
        script="scripts/archs4/clamp.R",
        common="scripts/archs4/common.R",
    output:
        rds=f"{{cell}}/CLAMPbase.rds",
        b=f"{{cell}}/CLAMPbase/B.csv",
        z=f"{{cell}}/CLAMPbase/Z.csv",
    log:
        f"{{cell}}/clampbase.log"
    params:
        fbm_prefix=lambda wildcards, input: input.fbm[: -len(".bk")],
        out_dir=lambda wildcards, output: os.path.dirname(output.rds),
        seed=A4_CFG["clamp"]["seed"],
    resources:
        mem_mb=lambda wc: a4_cov_retry_mem(a4_cov_fraction_of(wc.cell), 1),
        runtime=12000,
    wildcard_constraints:
        cell=A4_COV_CELL_PATTERN,
    conda: "clamp-analyses"
...
rule fit_bp_archs4_coverage:
    input:
        ...
        fbm=lambda wc: a4_cov_fbm(wc.fraction, wc.seed),
...
rule aggregate_bp_coverage:
    ...
    params:
        databases=",".join(A4_COV_DATABASES)
    resources:
        mem_mb=16000,
        runtime=1440,
    conda: "clamp-analyses"
...
rule publish_bp_model:
    input:
        model_dir=lambda wc: f"{a4_cov_model_dir(wc.dataset, 100, A4_COV_REFERENCE_SEED)}",
        validated=lambda wc: a4_cov_validated(wc.dataset, 100, A4_COV_REFERENCE_SEED),
        script="scripts/archs4/coverage/publish_bp_models.py",
    output:
        manifest=f"{A4_COV_BP_ROOT}/{{dataset}}/manifest.json",
    log:
        f"{A4_COV_BP_ROOT}/{{dataset}}/publish.log"
    params:
    ...
    resources:
        ...
        runtime=1440,
    wildcard_constraints:
        dataset="|".join(A4_COV_BP_DATASETS),
    conda: "clamp-analyses"
    shell:
        "python {input.script} --dataset {wildcards.dataset} "
        "--model-dir {input.model_dir} --validated {input.validated} "
...
rule audit_bp_coverage:
    input:
        script="scripts/archs4/coverage/audit_inputs.py",
        config="workflow/config/archs4.yaml",
    output:
        csv=f"{A4_COV_BIO}/audit/audit.csv",
        json=f"{A4_COV_BIO}/audit/audit.json",
    log:
        f"{A4_COV_BIO}/audit/audit.log"
    resources:
        mem_mb=4000,
        runtime=1440,
    conda: "clamp-analyses"
    shell:
        ...
        "--csv-out {output.csv} --json-out {output.json} > {log} 2>&1"


# Audits existing artifacts without refitting a missing model.
rule coverage_validate:
    input:
        rules.audit_bp_coverage.output.csv,


rule coverage_bp_cells:
    input:
        A4_COV_CELL_TARGETS,
