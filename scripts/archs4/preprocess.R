#!/usr/bin/env Rscript
#
# Stream the ARCHS4 HDF5 expression matrix into a filtered, z-scored FBM.
#
# Extracted from nbs/01_model_building/04_archs4/01_archs4_preprocess.ipynb.
# That notebook does not run as committed; the following were fixed here:
#   * `n_samples` was used to size the FBM before it was defined (it was only
#     assigned later, from the single-cell filter), so the backing file was
#     created with an undefined column count.
#   * the cleanFBM() call had lost its function name -- a bare `(fbm, ncores=)`.
#   * the unreadable-block fallback referenced an undefined `n_genes`.
#   * filterFBM() was handed `output_file`, which at that point still pointed at
#     data/archs4/gene_lengths.rds, so the filtered backing file would have been
#     written over the gene-length input.
...
# Keep only genes we have a length for. Symbols repeat in ARCHS4; the duplicates
# are collapsed by rowsum() below, which returns groups in sorted order.
...
for (i in seq_len(n_blocks)) {
  ...
  if (is.null(raw_block)) {
    # At least one block of ARCHS4 v2.5 cannot be read. Substitute near-zero
    # values so the column count stays right; those samples carry no signal and
    # are dropped by the variance filter downstream.
...
saveRDS(
  list(
    ...
    # Deliberately the full pre-filter lookup (one entry per gene that had a
    # length), not the kept subset: downstream code treats this as a symbol ->
    # length dictionary, and that is what the existing metadata_filtered.rds on
    # disk holds.
