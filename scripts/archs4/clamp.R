#!/usr/bin/env Rscript
#
# Fit CLAMPbase, then CLAMPfull with the multi-collection MSigDB prior, on a
# filtered ARCHS4 FBM.
#
# Extracted from nbs/01_model_building/04_archs4/{03_archs4_CLAMPbase,
# 04_archs4_CLAMPfull}.ipynb. The CLAMPfull notebook wrote CLAMPbase's loadings
# into the full result (`fullRes$Z <- data.frame(baseRes$Z)`), so its
# CLAMPfull/Z.csv was byte-identical to CLAMPbase/Z.csv. Nothing downstream read
# that output -- the shipped model comes from the coverage sweep, whose notebook
# is correct -- but the bug is fixed here rather than carried over.
...
stage <- required_arg(args, "stage")           # base | full
