...
> **Status — shipped un-wired.** `workflow/rules/phenoplier.smk` is **not** yet
> `include:`d by `workflow/Snakefile`; adding these files changes nothing on the
> existing DAG. Wiring (the `include:`/`configfile:`, and re-pointing the dataset
> symbols at `main`'s post-#36..#40 archs4-canonical layout) is left to the
> maintainer — see the header of `phenoplier.smk`. The GLS summaries these rules
> produce are the same per-model directories `workflow/rules/archs4_traits.smk`
> already consumes for its trait-recovery reports. **See
> [`RUN_SUMMARY.md`](RUN_SUMMARY.md) for the full list of models actually run and
> how to reproduce them on pico.**
...
| `verify_summary.py` | completeness gate for a GLS summary (2,366 phenotypes, no zero-p, degenerate-only NaN) |
| `final_models/` | the three CLAMPfull_bp **final production models** (register → GLS → store). See its README. |
| `saturation_k1728/` | ARCHS4 saturation study, k=1728 (17 models). See its README. |
| `coverage/` | ARCHS4 coverage study (21 models, pico reproduction). See its README. |
| `clampbase/` | the **CLAMPbase** family (finals + saturation + coverage) via the hardened per-model runner. See its README. |
| `RUN_SUMMARY.md` | **what was actually run** (86 models across 3 CLAMP families) + reproduce-on-pico index |
| `../../workflow/rules/phenoplier.smk` | the rule module (`phenoplier_traits`, `phenoplier_final`, per-group targets) — un-wired |
