...
#
# The earlier Hallmark saturation sweep left most of the grid's CLAMPbase fits
# on disk, but never produced rs50/rs75 (and rs1/k1728).  CLAMPbase is
# unsupervised, so the ones that do exist are reused as-is; this script only
# fills the gaps.
#
# Differences from scripts/archs4/clamp.R --stage base, which this is modelled
# on: K is passed directly with --clamp-k rather than read from a CLAMP_K.rds,
# and the output is published atomically from a temporary directory with a
# manifest, matching scripts/archs4/coverage/fit_clampfull.R so that validation,
# retries and the ORA rules all behave identically for both stages.
...
# The published layout is <out_dir>/CLAMPbase.rds + <out_dir>/CLAMPbase/{B,Z}.csv,
# so the whole cell directory is staged and moved into place at the end.
...
manifest <- list(
  ...
  # CLAMPbase is unsupervised: no prior is involved, which is why the fits from
  # the earlier Hallmark sweep are valid for the GO:BP campaign unchanged.
  prior = NULL,
...
if (dir.exists(out_dir)) {
  # The cell directory can already exist holding run_info.rds/CLAMP_K.rds from
  # the earlier sweep; merge into it rather than clobbering those.
